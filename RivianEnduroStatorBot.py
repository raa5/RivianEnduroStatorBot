...
def job():
    ...
    if (21 <= local_now.hour < 23) or (5 <= local_now.hour < 6):
        ...
        # Combine DataFrames
        df_combined_summary = pd.concat([df_20_summary, df_40_summary, df_50_summary, df_60_summary, df_65_summary, df_80_summary, df_100_summary, df_110_summary, df_210_summary], ignore_index=True)

        df_combined_summary["PARAMETER_NAME"] = df_combined_summary["ALARM_DESCRIPTION"].fillna(df_combined_summary["PARAMETER_NAME"])
        df_combined_summary.drop(columns=["ALARM_DESCRIPTION"], inplace=True)  # Remove old column
        ...
        if 'COUNT' in df_combined_summary.columns:
            df_combined_summary = df_combined_summary.sort_values(['COUNT'], ascending=False, ignore_index=True)

        # Aggregate total failures per station (without duplicates)
        df_sum = df_combined_summary.groupby("STATION_NAME")["COUNT"].sum().reset_index()

        # Merge unique product serial failures for Station 210
        if not df_210_unique_sn_summary.empty and "STATION_NAME" in df_210_unique_sn_summary.columns and "COUNT" in df_210_unique_sn_summary.columns:
            df_210_unique_sn_summary = df_210_unique_sn_summary.rename(columns={"COUNT": "FAIL_COUNT"})

            # Merge Station 210's unique product serial failures into df_sum
            df_sum_summary = df_sum_summary.merge(df_210_unique_sn_summary, on="STATION_NAME", how="left")

            # Replace total failure count with unique serial count for Station 210
            df_sum_summary["COUNT"] = df_sum_summary["FAIL_COUNT"].fillna(df_sum_summary["COUNT"])
        ...
        df_sum_summary = df_sum_summary.sort_values(['COUNT'], ascending=False, ignore_index=True)
    ...
    df_hairpin_origin_summary = pd.concat([df_40_hairpin_origin_summary, df_50_hairpin_origin_summary, df_65_hairpin_origin_summary], ignore_index=True)
    ...
    payload = {
        "blocks": [
            ...
            { "type": "divider" },  # Add a divider to separate sections clearly
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    # "text": "Fail COUNT by Parameter Summary for the shift: " + datetime.now().strftime('%Y-%m-%d %H:00')
                    "text": "*🚨Fail Count by Parameter - Shift Summary:* " + recorded_at_summary + " to " + (one_hour_before + timedelta(hours=1)).strftime('%H:00')
            ...
            },
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": "*Fails by Station Pareto - Shift Summary:*"
                }
            },
            {
                "type": "section",
            ...
            },
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": "*Fails by Hairpin Station - Shift Summary:*"
                }
            },
            {
                "type": "section",
