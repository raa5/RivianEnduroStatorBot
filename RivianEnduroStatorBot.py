...
#########################################################################################
    if (15 <= current_hour < 16) or (5 <= current_hour < 6):
        ...
        ########################################################################################
        # Query 65 - Fails by Hairpin Type - Summary
        ########################################################################################
        query_65_hairpin_type_summary = f"""
        with
        ...
        df_hairpin_type_summary = pd.read_sql(query_65_hairpin_type_summary, conn)
        ...
        df_hairpin_type_summary_str = df_to_table(df_hairpin_type_summary)
    ...
    if (15 <= current_hour < 16) or (5 <= current_hour < 6):
        payload["blocks"].extend(
            [
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": "```" + df_hairpin_type_summary_str + "```",
                    },
                },
                {"type": "divider"},  # Add a divider to separate sections clearly
            ]
